...
describe("reverseVaultForVoidedSale", () => {
  ...
  it("reverses a cash-only sale with one VOID_REVERSAL row and a balance decrement", async () => {
    ...
    // One reversal ledger row, negative amount.
    ...
    // Balance update is a decrement on the running cache.
  ...
  it("uses the branchId from the ledger row, not from input", async () => {
    // The whole point of the ledger-driven design: even if the order was
    // somehow moved to a different branch after the sale, the reversal
    // goes to the branch that originally received the money.
    harness.vaultTxFindMany.mockResolvedValue([
      {
  ...
  it("throws when SALE credits span multiple branches (data-integrity guard)", async () => {
    ...
    // No reversal rows or balance updates should have been written.
    expect(harness.vaultTxCreate).not.toHaveBeenCalled();
    expect(harness.branchVaultUpdate).not.toHaveBeenCalled();
  });
  ...
  it("propagates errors from the underlying transaction client", async () => {
    ...
    // Balance update is never reached if the ledger insert fails.
    expect(harness.branchVaultUpdate).not.toHaveBeenCalled();
  });

  it("queries only SALE-type rows for this order (verifies findMany filter shape)", async () => {
    harness.vaultTxFindMany.mockResolvedValue([]);
